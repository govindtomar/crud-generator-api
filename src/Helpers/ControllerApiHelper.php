        ...
        if (isset($tables)) {
            $controllerTemplate = str_replace(
                ...
                [
                    ...
                    ControllerApiHelper::getControllerFields($name, $fields),
                    ControllerApiHelper::getExtraTables($tables),
                    ControllerApiHelper::getCompactFunction($tables),
                    ControllerApiHelper::indexWithModel($tables),
                    ControllerApiHelper::joinSelectTables($name, $tables),
                    Helper::namespace(),
                    Helper::path(),
                    Helper::backslash(),
                    Helper::forslash(),
                    ControllerApiHelper::relativeModel($tables),
                    ControllerApiHelper::createExtraFunctionForToggle($name, $fields)
    ...
    public static function getControllerFields($name, $fields){
        ...
        foreach ($fields as $field) {
            ...
            if ($var[0] == 'select') {
                ...
                $field_name .=  "if(_____request->file('".$var[1]."')){\n\t\t\t\t";
                $field_name .= '_____'.Helper::modelNameSingularVar($name)."->".$var[1]."  = api_file_upload(_____request->file('".$var[1]."'), 'uploads');\n\t\t\t";
                $field_name .=  "}\n\t\t\t";
            }
